...
public:
    ...
    static void changeMedia(DiskIODriver *_driver)
    {
        driver = _driver;
    }

    static SdFile getroot()
    {
        return root;
    }
    ...
    static uint16_t permyriadDone()
    {
        ...
        if (isFileOpen() && filesize) {
            return sdpos / ((filesize + 9999) / 10000);
        }
        return 0;
    }
#endif
    ...
    static char* getWorkDirName()
    {
        workDir.getDosName(filename);
        return filename;
    }
    static SdFile& getWorkDir()
    {
        return workDir.isOpen() ? workDir : root;
    }
    ...
    static bool eof()
    {
        return getIndex() >= getFileSize();
    }

    // File data operations
    static int16_t get()
    {
        int16_t out = (int16_t)file.read();
        sdpos = file.curPosition();
        return out;
    }
    static int16_t read(void *buf, uint16_t nbyte)
    {
        return file.isOpen() ? file.read(buf, nbyte) : -1;
    }
    static int16_t write(void *buf, uint16_t nbyte)
    {
        return file.isOpen() ? file.write(buf, nbyte) : -1;
    }
    static void setIndex(const uint32_t index)
    {
        file.seekSet((sdpos = index));
    }

    // TODO: rename to diskIODriver()
    static DiskIODriver* diskIODriver()
    {
        return driver;
    }
